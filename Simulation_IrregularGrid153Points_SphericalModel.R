...
phi=seq (340, 340, by=20)
prob=rep(0, length(phi))
...
for (m in 1: length(phi))
     ...
     grid<-cbind(runif(153, 5, 85),runif(153, 35, 175))
     plot(grid, main="", xlab="", ylab="", pch=19)
     title(main=list("Grid 4 (Irregular 153Points)", cex=2, font =1) , xlab=list("x", cex=2, font =1), ylab=list("y", cex=2, font =1))
    ...
    #  Parameters of the spatial correlation
     ...
     covariance=cov.spatial(distance_coords, cov.pars = c(sigma2, phi[m]), cov.model = "sph") 
     #############################################
